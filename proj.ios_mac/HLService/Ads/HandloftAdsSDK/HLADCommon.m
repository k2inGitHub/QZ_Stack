...
+ (void)requestApiSuccess:(void(^)(NSString *adBannerUrl, NSString *adInterstitialUrl, NSString *adFixedInterstitialUrl))success failure:(void(^)(NSError *error))failure{
    ...
    NSDictionary *dic = @{@"app_bundle_id":[HLADDeviceHelper bundleName],@"app_version":[HLADDeviceHelper bundleShortVersion]};
    [manager GET:AdApiUrl parameters:@{@"content":[NSString JSONStringFromObject:dic]} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
